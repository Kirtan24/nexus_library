...
class ReservationsPage(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Tabs for different reservation states
        ...
        # Empty state labels
        self.active_empty_label = ctk.CTkLabel(
            self.active_tab,
            text="You don't have any active reservations",
            font=("Arial", 14)
        )

        self.notified_empty_label = ctk.CTkLabel(
            self.notified_tab,
    ...
    def load_reservations(self):
        ...
        # Load active reservations (waiting for availability)
        self.load_active_reservations()

        # Load notified reservations (books that became available)
    ...
    def load_active_reservations(self):
        ...
        if not reservations:
            self.active_empty_label.pack(pady=50)
            return
    ...
    def load_notified_reservations(self):
        ...
        if not reservations:
            self.notified_empty_label.pack(pady=50)
            return
    ...
    def create_reservation_card(self, parent, reservation, is_active):
        ...
        date_value = reservation['reservation_date'] if is_active else reservation['notification_date']

        ctk.CTkLabel(
            info_frame,
            text=f"{date_label}: {date_value.strftime('%Y-%m-%d %H:%M')}",
            anchor="w"
        ).pack(fill="x")
        ...
        if is_active:
            # For active reservations - remove button
            ctk.CTkButton(
                action_frame,
            ...
            # For notified reservations - borrow and dismiss buttons
            ctk.CTkButton(
                action_frame,
                text="Borrow Now",
                fg_color="#2E8B57",
